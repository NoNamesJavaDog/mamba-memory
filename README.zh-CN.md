...
37 维输入特征:
├── 15 规则信号（同上）
├──  8 语义嵌入（压缩 embedding，理解语义：Terraform ≈ 基础设施）
├──  8 上下文窗口（最近 20 条内容，判断话题延续 vs 话题转换）
└──  6 用户画像（追踪用户最常召回的主题，个性化重要性判断）
```

三种自进化信号：
- **用户纠正**（最强）：用户说"记住这个"→ 一次反向传播，<0.1ms
- **隐式正反馈**（自动）：被召回的记忆 → "存对了"→ 强化类似内容
- **隐式负反馈**（自动）：从未被召回就淘汰的记忆 → "存错了"→ 抑制类似内容

越用越准——用户不需要做任何事，系统自动从使用行为中学习。
...
5 折交叉验证，396 条样本（222 存 / 174 弃），14 个分类：

| 指标 | 规则引擎 | 神经门控 v2 |
|------|---------|------------|
| 准确率 | 88.6% ± 1.8% | **99.2% ± 1.0%** |
| 精确率 | 98.3% ± 2.3% | **99.6% ± 0.8%** |
| 召回率 | 81.1% ± 3.9% | **99.1% ± 1.7%** |
| F1 | 88.8% ± 2.1% | **99.3% ± 0.9%** |
| 置信度间距 | — | **0.948**（存储平均 0.976 vs 丢弃平均 0.029） |

5 折交叉验证确保数据反映真实泛化能力，不是背答案。
