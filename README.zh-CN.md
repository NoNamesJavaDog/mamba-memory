...
2. **自进化神经门控 v2**（可选）— 两层神经网络（37 维特征 → 16 隐藏神经元 ReLU → 1 输出 sigmoid），608 个参数：

```
66 维输入特征:
├── 15 规则信号（同上）
├──  4 否定/推迟检测（"到时候再决定" ≠ "我决定"）
├──  5 命令检测（terraform plan, psql -h 等可执行命令）
├──  4 代码/路径模式（文件路径、URI、配置文件、SQL）
├── 24 语义嵌入（压缩 embedding，理解语义）
├──  8 上下文窗口（最近 20 条，判断话题延续 vs 转换）
└──  6 用户画像（追踪用户最常召回的主题）
```

**否定/推迟检测（关键创新）**——最难的门控问题是包含动作词但意图相反的内容：

```
"我决定用PostgreSQL"        → 决策信号 ON + 推迟信号 OFF → 存储 ✓
"到时候再决定用什么数据库"     → 决策信号 ON + 推迟信号 ON  → 丢弃 ✓
"Haven't decided yet"       → 决策信号 ON + 推迟信号 ON  → 丢弃 ✓
```

**集成投票**——规则引擎 + 神经网络加权仲裁：双方同意则信任神经网络；分歧时根据置信度决定。

**三种自进化信号**：用户纠正（最强，<0.1ms）→ 隐式正反馈（被召回=存对了）→ 隐式负反馈（未召回就淘汰=存错了）。
...
| 否定检测 | 无 | 无 | **有** |
| 集成投票 | 无 | 无 | **规则+神经网络** |
| CV 准确率 | — | 98.7%±1.4% | **99.3%±0.6%** |
| 14 类全 100% | — | 13/14 | **14/14** |
...
5 折交叉验证，417 条样本（222 存 / 195 弃），14 个分类（含推迟/否定边界 case）：

| 指标 | 规则引擎 | 神经门控 v2 |
|------|---------|------------|
| 准确率 | 87.5% ± 1.7% | **99.3% ± 0.6%** |
| 精确率 | 95.2% ± 3.1% | **99.6% ± 0.8%** |
| 召回率 | 80.9% ± 5.3% | **99.0% ± 1.2%** |
| F1 | 87.2% ± 2.1% | **99.3% ± 0.6%** |
| 置信度间距 | — | **0.957**（存储平均 0.986 vs 丢弃平均 0.028） |

5 折交叉验证确保真实泛化能力。最后一折：**零错误，14 类全部 100%。**
...
| 写入过滤 | 无 | 无 | 无 | 神经门控（99.3% CV，自进化） |
| 遗忘模型 | 无 | 衰减 | 无 | 艾宾浩斯 + 间隔重复 |
| 学习型门控 | 无 | 无 | 无 | 有（神经网络 + 用户画像） |
| 知识图谱 | 有 | 有 | 无 | 有（类型化 + 推理） |
| 递归压缩 | 无 | 无 | 无 | 有（摘要链） |
