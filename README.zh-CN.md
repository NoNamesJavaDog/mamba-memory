...
每条输入经过 **三级决策流水线**，决定它是成为长期记忆、短期上下文、还是直接丢弃。
...
```
"你好"    → 0 分, 丢弃     "ok"      → 0 分, 丢弃
"嗯嗯"    → 0 分, 丢弃     "こんにちは" → 0 分, 丢弃
"好的收到"  → 0 分, 丢弃     "see you"  → 0 分, 丢弃
```

覆盖：问候、确认、告别、表情反应、单词回复，支持中英日韩。这些永远不进入任何记忆层。

#### 第二级：重要性评分（规则引擎 + 学习型分类器）
...
| 信号 | 示例 | 为什么重要 |
|------|------|-----------|
| 决策 | "我决定用Docker"、"switch to PostgreSQL" | 改变系统状态 |
| 偏好 | "我喜欢用VS Code"、"prefer tabs" | 个性化行为 |
| 纠正 | "不对，端口是8443"、"actually it's..." | 修正错误知识 |
| 带数据的事实 | "IP: 192.168.1.1, port 5432" | 具体可检索信息 |
| 显式请求 | "记住这个"、"don't forget" | 用户明确要求持久化 |
| 时间安排 | "每天凌晨3点备份"、"daily at 3am" | 运维调度 |
| 配置参数 | "maxmemory 256MB"、"rate limit 100/min" | 系统参数 |
| 行动项 | "下一步部署到生产"、"TODO: add tests" | 待办任务 |

**会被丢弃的内容**（低价值信号）：

| 信号 | 示例 | 为什么过滤 |
|------|------|-----------|
| 问候 | "你好"、"hello"、"早上好" | 零信息量 |
| 确认 | "好的"、"ok"、"收到" | 无新知识 |
| 闲聊 | "天气真好"、"周末有计划吗" | 不可执行 |
| 填充 | "嗯嗯"、"哈哈"、"..." | 噪声 |
| 模糊回复 | "可能吧"、"我想想" | 无确定性 |
...
通过评分的内容进入记忆系统，但 **放在哪里** 取决于上下文：

```
分数 >= 0.25 + 新主题    → L2 新槽位（长期认知状态）
分数 >= 0.25 + 相似主题  → L2 更新已有槽位（知识合并）
分数 >= 0.20 + 新主题    → L2 中等置信度槽位
分数 < 0.20             → 仅留在 L1（短期窗口）
force=True              → 直接写入 L2（跳过所有评分）
```

进入 L2 后，记忆 **不是永久的** — 它们竞争生存：
- **活跃记忆**（经常被召回）通过复习效应越来越强
- **被忽略的记忆** 随时间自然衰减（艾宾浩斯曲线）
- **最弱的记忆** 被淘汰到 L3 冷存储（仍可搜索，只是更慢）
- **所有槽位都满且活跃度高？** 低价值新内容被拒绝（饱和保护）

这意味着系统会自然收敛到存储用户 **真正使用的内容**，而不仅仅是写入时看起来重要的。
